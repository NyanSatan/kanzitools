...
Previously known as **kanzictl**. Now it's a pack of 6 tools for interacting with **Kanzi**, its' derivatives (**Nova**/**Koba**) and in some cases other probes (**Chimp**, **Koko**, etc.), as well as their bootloaders
...
**UDT** doesn't have the button (or it's unreachable due to its' special enclosure), so **probeenterdfu** in conjuction with patched `astrisprobed` might be handy
...
Using `make.sh`. This program does depend on **libAstrisAPI**, which must be installed at `/usr/local/lib` (comes with Astris since Tigris, but some builds may lack it - e.g. Sunburst23A313)
...
This one allows you to interact with **KFL** (**K**anzi **F**raud **L**ibrary) functionality of your **Kanzi**/**Nova** (**Koba** and **Chimp** are not currently supported). To learn more about the subject, please read [this](https://nyansatan.github.io/lightning-snr/)

### Building

Using Xcode. This tool does depend on **libAstrisAPI**, which must be installed at `/usr/local/lib` (comes with Astris since Tigris, but some builds may lack it - e.g. Sunburst23A313)
...
***Warning**: this tool hasn't been tested with modern versions of Astris and macOS, so be careful! Nowadays you shall better [patch firmware](https://nyansatan.github.io/snr-udt-fw-patching/) of your probe instead*
...
***Warning**: be extremely careful with this one. Nowadays you shall better [patch firmware](https://nyansatan.github.io/snr-udt-fw-patching/) of your probe instead*
